...
flags.DEFINE_float("margin",0.4,"CLS_p 和正确答案和错误答案的距离")
...
def compute_loss(start_logits,end_logits,answer_type_logits,start_positions,end_positions,answer_type_positions,mode):
    ...
    if(mode=="average_combination"):
        return compute_average_combination_loss(start_logits,end_logits,answer_type_logits,start_positions,end_positions,answer_type_positions)
...
def compute_combination_loss(start_logits,end_logits,answer_type_logits,start_positions,end_positions,answer_type_positions):
    # basic loss
    start_loss = compute_position_loss(start_logits, start_positions)
    end_loss = compute_position_loss(end_logits, end_positions)
    answer_type_loss = compute_label_loss(answer_type_logits, answer_type_positions)
...
def compute_average_combination_loss(start_logits,end_logits,answer_type_logits,start_positions,end_positions,answer_type_positions):
    # basic loss
    start_loss = compute_position_loss(start_logits, start_positions)
    end_loss = compute_position_loss(end_logits, end_positions)
    answer_type_loss = compute_label_loss(answer_type_logits, answer_type_positions)
    ...
    # max no answer loss
    extra_start_loss = compute_position_average_combination_loss(start_logits,start_positions)
    extra_end_loss = compute_position_average_combination_loss(end_logits,end_positions)
...
def compute_old_loss(start_logits,end_logits,answer_type_logits,start_positions,end_positions,answer_type_positions):
    ...
    # max no answer loss
    extra_start_loss = compute_position_old_loss(start_logits,start_positions)
    extra_end_loss = compute_position_old_loss(end_logits,end_positions)
...
def compute_position_hinge_loss(logits,one_hot_positions):
    ...
    positive_loss =  tf.reduce_sum(positive_L*has_answers) /(tf.reduce_sum(has_answers)+0.001)
...
def compute_position_max_no_answer_loss(logits,one_hot_positions):
    ...
    #negative loss 让CLS_p尽可能大
    negative_loss =  tf.reduce_mean(-CLS_p) 
    
    return positive_loss+negative_loss


# combination of hinge loss and max_no_answer_loss
def compute_position_combination_loss(logits,one_hot_positions):
    # 归一化处理
    logits = tf.nn.log_softmax(logits)
    ...
    #max_no_answer_loss
    max_no_answer_loss = tf.reduce_mean(-CLS_p)
    
    return positive_loss+negative_loss+max_no_answer_loss

# combination of hinge loss and max_no_answer_loss
def compute_position_average_combination_loss(logits,one_hot_positions):
    # 归一化处理
    logits = tf.nn.log_softmax(logits)
    ...
    #positive
    # CLS_P + margin < positive_logits 找到最小的positive logits
    positive_H = tf.reduce_sum(margin+CLS_p - positive_logits,axis = -1) # 找到小于CLS+margin 最多处的positive_logits 
    positive_L = tf.nn.relu(positive_H)
    positive_loss =  tf.reduce_sum(positive_L*has_answers) /(tf.reduce_sum(has_answers)+0.001)
    
    #negative
    # CLS_P - margin > negative_logits
    negative_H = tf.reduce_sum(negative_logits + margin - CLS_p,axis = -1)
    negative_L = tf.nn.relu(negative_H)
    negative_loss =  tf.reduce_mean(negative_L) 
...
def compute_position_old_loss(logits,one_hot_positions):
    # 归一化处理
    logits = tf.nn.log_softmax(logits)
