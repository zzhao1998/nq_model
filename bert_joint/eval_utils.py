...
NQLabel = collections.namedtuple(
    ...
    [
        ...
        'short_score_list'  # The prediction score for each short answer span.
    ])
...
def is_null_span_list(span_list, score_list, threshold = 0):
    ...
    if not span_list or all([span.is_null_span() for span in span_list]):
        return True
    for span, score in zip(span_list, score_list):
        if score > threshold:
            return False
    return True
...
def gold_has_short_answer(gold_label_list):
    ...
    # FLAGS.short_non_null_threshold modified to 1
    FLAGS.short_non_null_threshold = 1

    gold_has_answer = gold_label_list and sum([
        ((not is_null_span_list(label.short_answer_span_list, label.short_score_list)) or
    ...
    for single_prediction in predictions['predictions']:
        ...
        if 'short_answers' in single_prediction:
            for short_item in single_prediction['short_answers']:
                ...
                short_score_list += [short_item['score']]
        ...
        pred_item = NQLabel(
            ...
            short_score_list=short_score_list)
        ...
        for line in input_file:
            ...
            for annotation in json_example['annotations']:
                ...
                gold_label = NQLabel(
                    ...
                    short_answer_span_list=short_span_list,
                    long_score=0,
                    short_score_list=[1] * len(short_span_list),
