...
from answer_predict_utils import get_prediction
...
def compute_predictions(example):
  ...
  for unique_id, result in example.results.iteritems():
    ...
    unanswerable_p = result["start_logits"][0] + result["end_logits"][0]
    ...
    answers,score = get_prediction(start_logits,end_logits,unanswerable_p,max_answer_length,mode = "greedy")
    summary.answers = answers

    summary.score = score
    for span in summary.answers:
        span[0] =token_map[span[0]]
        span[1] = token_map[span[1]] + 1

    predictions.append((score, summary))

  score, summary= sorted(predictions, reverse=True)[0]
