...
flags.DEFINE_bool("do_print", True, "zzh add ")
...
def get_first_annotation(e):
  ...
  Returns:
    ...
    annotated_sa: (tuple) char offset of the start token of the first short answer and end token
        of the last short answer. The end token is exclusive. 
  """
  ...
  for a in positive_annotations:
    if a["short_answers"]:
      ...
      for answer in a["short_answers"]:
        start_token =answer["start_token"]
        end_token = answer["end_token"]
        answer_spans.append( [token_to_char_offset(e, idx, start_token),
  ...
  for a in positive_annotations:
    if a["short_answers"]:
      ...
      return a, idx, (token_to_char_offset(e, idx, start_token),
                      token_to_char_offset(e, idx, end_token) - 1),answer_spans
...
def create_example_from_jsonl(line):
  ...
  annotation, annotated_idx, annotated_sa ,annotated_answer_spans= get_first_annotation(e)
  #print("annotated_answer_spans")
  #print(annotated_answer_spans)
  #print(annotated_sa)
  ...
  example = {
      ...
      "answer_spans" : annotated_answer_spans
  }
  ...
  for context in context_list:
    ...
    if context["id"] == annotated_idx:
      ...
      for span in example["answer_spans"]:
        span[0] += offset
        span[1] += offset
        pass
  ...
  #print(example["answer_spans"])
  return example

#�����ԭ���е�answerת��Ϊ(type,text,offset)�Ľṹ
...
def read_nq_entry(entry, is_training):
  ...
  for i, question in enumerate(entry["questions"]):
    ...
    answer = None
    answer_spans = [] 
    if is_training or FLAGS.do_print:
      ...
      for span in entry["answer_spans"]:
        span_start = char_to_word_offset[span[0]]
        span_end = char_to_word_offset[span[1]]
...
def convert_single_example(example, tokenizer, is_training):
  ...
  for span in example.answer_spans:
    ...
    print(span)
    span_start = orig_to_tok_index[span[0]]
    span_end = 0
    if example.end_position < len(example.doc_tokens) - 1:
      span_end = orig_to_tok_index[span[1] + 1] - 1
    else:
      span_end = len(all_doc_tokens) - 1
    mid_answer_spans.append((span_start,span_end))
  ...
  for (doc_span_index, doc_span) in enumerate(doc_spans):
    ...
    if is_training or FLAGS.do_print:
      ...
      if ((not contains_an_annotation) or
        ...
        # calculate final answer_span
        for span in mid_answer_spans:
          span_start = span[0] - doc_start + doc_offset
          span_end = span[1] - doc_start+ doc_offset
          answer_spans.append([span_start,span_end])
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    #all_results eval_example eval_features
    ...
    for i in range(len(eval_examples)):
      #print(eval_examples[i].example_id)
