...
# author : zzh
# 1. support different mode (multi-answers / single answer) training
# 2. support different trainning mode (predict start end/ predict every word)
# 3. support different loss
# 4. concat adjoining answers
...
flags.DEFINE_bool("is_multi_combination", True,"决定是否把多答案的例子转化为多个多跳例子")
# 如果设置为0 则意味着不会相连 否则当两个答案距离小于answer_interval时，则会被合并为1个，这里的距离表示的token距离

# model type
flags.DEFINE_string("model_mode", None, "the type of model")
# basic 基础模型
# mask 使用含mask的模型

# loss type
flags.DEFINE_string("loss_mode", None, "the type of loss")
...
class NqExample(object):
  ...
  def __init__(self,
    ...
    # add short answers : a list of short answers' span
...
def get_short_answers_list(e,idx,a):
  # e 整个数据  idx 正确长答案的编号  a 一个annotattion
  # 这里要求原始的答案是按顺序排列的
  # 返回一个已经转化为char offset的short_answers : [(char_start,char_end)...]
  # 这里保证答案非空
  ...
  origin_short_answers = a["short_answers"]
  
  start_token = origin_short_answers[0]["start_token"]
  end_token = origin_short_answers[0]["end_token"]

  n = len(origin_short_answers)
  for i in range(1,n):
    span = origin_short_answers[i]
    current_start_token = span["start_token"]
    current_end_token = span["end_token"]
    if current_start_token - end_token > FLAGS.answer_interval:
      #这个是不连接的情况
      #把上一个加入到列表中
      short_answers.append( [token_to_char_offset(e, idx, start_token),     
...
def get_first_annotation(e):
  ...
  Returns:
    ...
    annotated_sa: (list of tuples) char offset of the start and end token
        of the short answer. The end token is exclusive.
  """
  # author:zzh
  # 关键函数 读取一个json 读取其中的annotations然后把他转化为具体的答案
  # 这里需要增加支持多答案 以及需要支持答案连接,输出为 a,idx,[(char_start,char_end)...]
  # e 对应整个json
  ...
  for a in positive_annotations:
    if a["short_answers"]:
      idx = a["long_answer"]["candidate_index"] # idx对应的是正确的长答案的index

      short_answers = get_short_answers_list(e,idx,a) 
      # 这里是一个列表，对应的是token_to_char_offset  可以把某个位置(token)转换为正确长答案中的char位置
      return a,idx, short_answers
...
def create_example_from_jsonl(line):
  ...
  # annotated_sa: short answer start and end char offsets, (-1, -1) if null. 这里是在长答案中的位置
  ...
  # 修改后这里的short answer 是一个list
  if len(annotated_sa) != 0:
    answer["input_text"] = "short"
    span_text = get_candidate_text(e, annotated_idx).text #长答案对应片段
    for char_offset in annotated_sa:
      answer["spans_text"].append(span_text[char_offset[0]:char_offset[1]])
    answer["spans"] = annotated_sa
  ...
  # span_texts 对应整个片段 
    ...
    if FLAGS.ignore_long_answer == False:
      long_answer_text = get_candidate_text(e, annotated_idx).text
      answer["spans_text"] = [long_answer_text]
      answer["spans"].append([0,len(long_answer_text)])
    #answer["span_start"] = 0
  ...
  #这里实际上是在原有的基础上加入了context信息，然后需要重新的设置span的start和end
  single_map = []
  single_context = []
  offset = 0
  ...
  example["contexts"] = " ".join(single_context)#对应包含分段信息的完整文本
  ...
  if annotated_idx in context_idxs:
    # 数量检查
    assert len(answer["spans"]) == len(answer["spans_text"])
    
    #顺序一致性检查
    L = len(answer["spans"])
    for i in range(L):
      span = answer["spans"][i]
      text = answer["spans_text"][i]
      expected = example["contexts"][span[0]:span[1]]
      assert expected == text, (expected, answer["spans_text"])
  
  return example

def span_in_context(spans,L):
  for span in spans:
    if span[0]>=L or span[1]>L:
      return False
  return True
#这里把原题中的answer转化为(type,text,offset)的结构
def make_nq_answer(contexts, answer):
  ...
  short_answers = [] # 返回集合
  if (answer["candidate_id"] == -1 or (not span_in_context(answer["spans"],len(contexts)))):
    ...
    for span in answer["spans"]:
      ...
      answer = Answer(answer_type, text=contexts[start:end], offset=start)
      short_answers.append(answer)
  return answer_type,short_answers


def read_nq_entry(entry, is_training):
  """Converts a NQ entry into a list of NqExamples."""
  # 这里的entry 对应的是create_example_from_jsonl中提供的example
  # 现在要把他转化为一列的NqExamples 这里实际上只会转化为一个
  def is_whitespace(c):
    return c in " \t\r\n" or ord(c) == 0x202F


  examples = []
  contexts_id = entry["id"]   #问题和文本本来的id
  contexts = entry["contexts"]   #文本内容
  doc_tokens = []
  char_to_word_offset = []
  ...
  #}
  questions = []
  for i, question in enumerate(entry["questions"]): # 这里实际上只有一个问题
    ...
    if is_training :
      ...
      answer_type,short_answers=make_nq_answer(contexts,answer_dict)
...
def convert_examples_to_features(examples, tokenizer, is_training, output_fn):
  ...
  # 这里把第一部的nqexample生成feature
  # 这里的feature相当于切分后的example
  ...
  for example in examples:
    ...
    #这里为每一个feature在example基础上创建一个id
    #一个example对应多个feature
...
def convert_single_example(example, tokenizer, is_training):
  ...
  # ANSWER
  # 这里需要把answer span从origin转化为token

  orig_spans = example.spans
  #print(orig_spans)
  tok_spans = []
  if is_training:
    for orig_span in orig_spans:
      tok_start = orig_to_tok_index[orig_span[0]]
      if orig_span[1] < len(example.doc_tokens) - 1:
        tok_end = orig_to_tok_index[orig_span[1] + 1] - 1
      else:
        tok_end = len(all_doc_tokens) - 1
      tok_spans.append([tok_start,tok_end])
  #print(tok_spans)
  ...
  #print("SLICE_FEATURE____________________________________")
  #print(len(doc_spans))
  for (doc_span_index, doc_span) in enumerate(doc_spans):
    # 每一个doc就是一个符合最大长度的序列

    # tokens CLS QUERY SEP | CONTEXT SEP
    ...
    answer_type = None
    answer_text = []
    inside_tok_spans = [] # 这里构建inside_tok_spans 表示在这个切片中的所有答案
    
    if is_training:
      doc_start = doc_span.start
      doc_end = doc_span.start + doc_span.length - 1
      # 这里需要使用token spans 进行筛选
      ...
      for span in tok_spans:
        ...
        if (start >= doc_start and end <= doc_end):
          # 这里的inside_span需要相对于切片进行转化
          doc_offset = len(query_tokens) + 2
          converted_start = start - doc_start + doc_offset
          converted_end = end - doc_start + doc_offset
          inside_tok_spans.append([converted_start,converted_end])
          contains_an_annotation= True
      #print(inside_tok_spans)
      if ((not contains_an_annotation) or
          example.answer_type == AnswerType.UNKNOWN):
      ...
      for span in inside_tok_spans:

        start = span[0]
        end = span[1]
        
        answer_text.append( " ".join(tokens[start:(end + 1)]))
      
      spans_combinations,mask_combinations = get_span_combination(inside_tok_spans,FLAGS.is_multi_combination)
      #这里is_multi_combination
      for i in range(len(spans_combinations)):
        #对于每一种组合，创建一个feature

        #三个oneHot
        start_position_onehot = [0]*FLAGS.max_seq_length
        end_position_onehot = [0]*FLAGS.max_seq_length
        mask_onehot = [0]*FLAGS.max_seq_length

        #组合
        
        spans_combination = spans_combinations[i]
        mask_combination = mask_combinations[i]
        #print("combination_FEATURE____________________________________")
        #print(spans_combination)
        #print(mask_combination)
        for span in spans_combination:
          #这里把end_position和start_position
          start = span[0]
          end  = span[1]
          start_position_onehot[start] = 1
          end_position_onehot[end] = 1
        for span in mask_combination:
          start = span[0]
          end = span[1]
        ...
        #print(start_position_onehot)
        #print(end_position_onehot)
        #print(mask_onehot)
        #如果mask已经把所有的答案遮盖住了，则无答案,预测unknown
        cur_answer_type =  copy.deepcopy(answer_type)
        if(len(spans_combination)==0):
          cur_answer_type = AnswerType.UNKNOWN
          start_position_onehot[0] = 1
          end_position_onehot[0] = 1
        #print(cur_answer_type)
        feature = InputFeatures(
          ...
          inside_answer_spans=inside_tok_spans,
          start_position_onehot = start_position_onehot,
          end_position_onehot = end_position_onehot,
          mask_onehot = mask_onehot,
          answer_text=answer_text,
          answer_type=cur_answer_type)

        features.append(feature)
      #这里要构造一个列表，使得每一种答案组合都被枚举一次
    else:#predict 
      #直接给出feature就可以
      ...
      feature = InputFeatures(
        ...
        segment_ids=segment_ids,
        inside_answer_spans=inside_tok_spans,
        start_position_onehot = None,
        end_position_onehot = None,
        mask_onehot = mask_onehot,
        answer_text=answer_text,
        answer_type=answer_type)
      #print(input_mask)
...
def get_span_combination(spans,is_multi_combination):
    if(is_multi_combination == True):
      return get_all_span_combination(spans)
    else:
      #这样就相当于每一个样本还是只产生一个结果
      return [spans],[[]]

def get_all_span_combination(spans):
    if(len(spans)==0):
        return [],[]
    span_combinations=[]
    mask_combinations=[]
    ...
    for i in range(num_combinations):
        code = binary(i,length)
        #print(code)
        span_combination = []
        mask_combination = []
        for j in range(length):
            if code[j]=='0':
                mask_combination.append(spans[j])
            else:
                span_combination.append(spans[j])
        span_combinations.append(span_combination)
        mask_combinations.append(mask_combination)
    return span_combinations,mask_combinations
    ...
    for input_feature in input_features:
      ...
      if self.is_training:
        features["start_positions"] = create_int_feature(
            input_feature.start_position_onehot)
        features["end_positions"] = create_int_feature(
            input_feature.end_position_onehot)
        features["mask_positions"]=create_int_feature(
            input_feature.mask_onehot)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    # 这个需要在预测的时候需要根据已经选出的答案进行确定
    mask_positions = features["mask_positions"]
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      total_loss = compute_loss(start_logits,end_logits,answer_type_logits,start_positions,end_positions,answer_types,mode=FLAGS.loss_mode)
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
  ...
  if is_training:
    name_to_features["start_positions"] = tf.FixedLenFeature([seq_length], tf.int64)
    name_to_features["end_positions"] = tf.FixedLenFeature([seq_length], tf.int64)
...
# 这个只用来在eval的时候，把一个输入的东西 写成Feature
class FeatureWriter(object):
  ...
  def process_feature(self, feature):
    ...
    #feature.mask_onehot
    features["mask_positions"] = create_int_feature(feature.mask_onehot)
...
#这里get_prediction用来计算具体的答案位置 需要修改
from answer_predict_utils import compute_predictions
"""
def compute_predictions(example):
  ...
  for unique_id, result in example.results.iteritems():
    ...
    prediction_record_list.extend(get_prediction(start_logits,end_logits,answer_type_logits,max_answer_length,mode = "greedy"))
    def get_score(prediction_record):
      return prediction_record.score
    best_prediction_record= [sorted(predictions,key = get_Score ,reverse=True)[0]]
  ...
  def get_short_answers_output(answers):
    # 这个用以获取输出结果
  ...
  summary.predicted_label = {
      ...
      "short_answers": get_short_answers_output(summary.answers),
...
def main(_):
  ...
  train_data_list = None #读取的输入文件列表
  if FLAGS.do_train:
    # 这里需要读取输入文件
    train_filename = FLAGS.train_precomputed_file
    num_train_features_total = FLAGS.train_num_precomputed
    train_data_list,num_train_features_total = file_utils.read_train_data(train_filename,num_train_features_total)
    # 返回一个文件list
     
    num_train_steps_total = int(num_train_features_total / FLAGS.train_batch_size *FLAGS.num_train_epochs) +20
    # 这个+20是防止出现 因为某个数据块的某个batch不足,导致最后总体的训练步数要大于我们指定的训练步数
    tf.logging.info("***** Running training on precomputed features *****")
    tf.logging.info("  Num split examples = %d", num_train_features_total)
    tf.logging.info("  Batch size = %d", FLAGS.train_batch_size)
    tf.logging.info("  train_step_total = %d", num_train_steps_total)
    num_warmup_steps = int(num_train_steps_total * FLAGS.warmup_proportion)

  print("Setting----------------------------------------")
  print("model_mode:"+FLAGS.model_mode)
  print("loss_mode:"+FLAGS.loss_mode)
  print("predict_mode:"+FLAGS.predict_mode)
  model_fn = model_fn_builder(
      bert_config=bert_config,
      init_checkpoint=FLAGS.init_checkpoint,
      learning_rate=FLAGS.learning_rate, 
      num_train_steps=num_train_steps_total , #这个值只和数据衰减有关
  ...
  if FLAGS.do_train:
    ...
    for i in range(FLAGS.num_train_epochs):
      tf.logging.info("  EPOCH %d", i)
      #train_filename = FLAGS.train_precomputed_file # 这个既可能是一个名字，也可能是一个
      for record in train_data_list:
      
        train_filename = record[0]
        num_train_steps =record[1]/FLAGS.train_batch_size +1
        current_max_steps += num_train_steps
        tf.logging.info("  filename = %s", train_filename)
        tf.logging.info("  Num steps = %d", num_train_steps)
        train_input_fn = input_fn_builder(
          input_file=train_filename,
          seq_length=FLAGS.max_seq_length,
          is_training=True,
          drop_remainder=True)
        estimator.train(input_fn=train_input_fn, max_steps=current_max_steps)
  # 预测工作
  if FLAGS.do_predict:
